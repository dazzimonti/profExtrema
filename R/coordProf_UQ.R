...
#' }
#' @export
coordProf_UQ = function(kmModel,threshold,allResMean=NULL,quantiles_uq=c(0.05,0.95),options_approx=NULL,options_sims=NULL,plot_level=0,plot_options=NULL,return_level=1){
  ...
  if(is.null(options_sims)){
    options_sims<-list(algorithm="B", lower=rep(0,kmModel@d), upper=rep(1,kmModel@d),
                       batchsize=120, optimcontrol = list(method="genoud",pop.size=100,print.level=0),
                       integcontrol = list(distrib="sobol",n.points=1000),
                       nsim=300)
    options_sims$integration.param = integration_design(options_sims$integcontrol,kmModel@d,options_sims$lower,options_sims$upper,kmModel,threshold)
  ...
  timeIn<-get_nanotime()
  m_dist<-optim_dist_measure(model = kmModel,threshold = threshold,
  ...
  if(plot_level>0){
    ...
    plot(m_dist$value,type='o',main="Optimized criterion",ylab="y",xlab="Iteration")
    if(plot_options$save)
      dev.off()
  }
  ...
  nugget.sim=1e-6
  type="UK"
  simu_points<-m_dist$par

  some.simu <- simulate_km(object=kmModel,nsim=options_sims$nsim,newdata=simu_points,nugget.sim=nugget.sim,
  ...
  profSups<-array(NA,dim = c(kmModel@d,options_approx$fullDesignSize,options_sims$nsim))
  profInfs<-array(NA,dim = c(kmModel@d,options_approx$fullDesignSize,options_sims$nsim))
  tApprox1ord<-rep(NA,options_sims$nsim)

  d<-kmModel@d

  F.mat <- model.matrix(object=kmModel@trend.formula, data = data.frame(rbind(kmModel@X,simu_points)))
  K <- covMatrix(object=kmModel@covariance,X=rbind(kmModel@X,simu_points))$C
  T.mat <- chol(K)
  for(i in seq(options_sims$nsim)){

    g_uq_spec<-function(x){
      return(g_uq(x=x,realization=some.simu[i,],kmModel = kmModel,simupoints = simu_points,F.mat = F.mat,T.mat = T.mat))
    }
    g_uq_der_spec<-function(x){
      return(g_uq_deriv(x=x,realization=some.simu[i,],kmModel = kmModel,simupoints = simu_points,T.mat = T.mat,F.mat = F.mat))
    }
    if(i%%10==0){
      cat("Realization ",i,"\n")
    }
    ...
    timeIn<-get_nanotime()
    temp_1o<-approxMaxMin(f = g_uq_spec,fprime = g_uq_der_spec,threshold = threshold,d = kmModel@d,opts = options_approx)
    ...
    profSups[,,i]<-t(temp_1o$res$max)
    profInfs[,,i]<-t(temp_1o$res$min)
  }

  prof_quantiles_approx<-list()
  for(i in seq(length(quantiles_uq))){
    prof_quantiles_approx[[i]]<-list(res=list(min=matrix(NA,nrow = options_approx$fullDesignSize,ncol = kmModel@d),
                                              max=matrix(NA,nrow = options_approx$fullDesignSize,ncol = kmModel@d)))
  }
  names(prof_quantiles_approx)<-quantiles_uq
  ...
  for(j in seq(length(quantiles_uq))){
    for(coord in seq(d)){
      prof_quantiles_approx[[j]]$res$max[,coord]<-apply(profSups[coord,,],1,function(x){return(quantile(x,quantiles_uq[j]))})
      prof_quantiles_approx[[j]]$res$min[,coord]<-apply(profInfs[coord,,],1,function(x){return(quantile(x,quantiles_uq[j]))})
    }
    ccPP[[j]]<-getChangePoints(threshold = threshold,allRes = prof_quantiles_approx[[j]])
  }
  ...
  if(plot_level>=1){
    ...
    for(coord in seq(d)){
      title_string<-paste("Coordinate",colnames(kmModel@X)[coord])
      if(!is.null(allResMean)){
        ylimTemp<-range(c(range(profSups[coord,,]),range(profInfs[coord,,]),
        ...
        ylimTemp<-range(c(range(profSups[coord,,]),range(profInfs[coord,,]),
                          range(prof_quantiles_approx$`0.5`$res$min[,coord]),
                          range(prof_quantiles_approx$`0.5`$res$max[,coord])))
        plot(plot_options$design[,coord],prof_quantiles_approx$`0.5`$res$min[,coord],ylim=ylimTemp,type='l',main=title_string,
             xlab=expression(eta),ylab="f",lty=1,cex.main=3,cex.lab=2.5,cex.axis=1.8)
      }
      for(i in seq(options_sims$nsim)){
        dd<-seq(0,1,,length.out = length(profInfs[coord,,i]))
        lines(dd, profSups[coord,,i],col=adjustcolor('darkolivegreen3',alpha.f=0.5),lwd=0.2)
        lines(dd, profInfs[coord,,i],col=adjustcolor('darkolivegreen3',alpha.f=0.5),lwd=0.2)
      }
      if(plot_options$qq_fill){
        polygon(c(plot_options$design[,coord], rev(plot_options$design[,coord])),
                c(prof_quantiles_approx[[2]]$res$min[,coord],
                  rev(prof_quantiles_approx[[1]]$res$min[,coord])),
                col = adjustcolor("red",alpha.f=0.5), border = NA)
        polygon(c(plot_options$design[,coord], rev(plot_options$design[,coord])),
                c(prof_quantiles_approx[[2]]$res$max[,coord],
                  rev(prof_quantiles_approx[[1]]$res$max[,coord])),
      ...
      for(j in seq(length(quantiles_uq))){
        lines(plot_options$design[,coord],prof_quantiles_approx[[j]]$res$max[,coord],lty=j,col=1,lwd=1.5)
        lines(plot_options$design[,coord],prof_quantiles_approx[[j]]$res$min[,coord],lty=j,col=1,lwd=1.5)
      ...
      if(!is.null(allResMean)){
        ...
        lines(plot_options$design[,coord],prof_quantiles_approx$`0.5`$res$max[,coord],lty=1)
        lines(plot_options$design[,coord],prof_quantiles_approx$`0.5`$res$min[,coord],lty=1)
      }
  ...
  results<-list(profSups=profSups,profInfs=profInfs,prof_quantiles_approx=prof_quantiles_approx)

  if(return_level==1){
    return(results)
  }else{
    times<-list(tSpts=timeMdist,tApprox1ord=tApprox1ord)
    results$more<-list(simuls=some.simu,times=times,sPts=m_dist)
    return(results)
  }
